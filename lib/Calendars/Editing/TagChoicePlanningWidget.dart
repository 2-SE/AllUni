...
class TagsChoicePlanningWidget extends StatefulWidget {
  ...
  const TagsChoicePlanningWidget({
    Key? key,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.calendarAppointment != null) {
      ...
      List<String> customTags = planningTagsProvider.activatedTags
          ...
          .toList();
      //---------- prints ----------
      print("-- activatedTags :  ${planningTagsProvider.activatedTags}");
      print("-- customTag :      $customTags");
    }

    // context.watch<PlanningTagsProvider>().tags.forEach((tagItem) {
    //   if (widget.calendarAppointment != null &&
    //       widget.calendarAppointment!.tagsNames.contains(tagItem.label)) {
    //     planningTagsProvider.changeTagValue(tagItem);
    //   }
    // });
    // print(planningTagsProvider.activatedTags);
...
//TODO - CUSTOM TAG(gerer le problème de validation)
                  InputChip(
